export const metadata = {
  "title": "Event Properties Conversion",
  "description": "Learn how Datablit converts event properties to snake_case and flattens nested objects. Understand property naming conventions and data transformation rules.",
  "alternates": {
    "canonical": "https://docs.datablit.com/docs/data-ingestion/event-properties-conversion"
  },
  "openGraph": {
    "title": "Event Properties Conversion",
    "description": "Learn how Datablit converts event properties to snake_case and flattens nested objects. Understand property naming conventions and data transformation rules.",
    "url": "https://docs.datablit.com/docs/data-ingestion/event-properties-conversion",
    "siteName": "Datablit Documentation",
    "type": "article",
    "images": [
      {
        "url": "/icon.svg",
        "width": 1200,
        "height": 630,
        "alt": "Event Properties Conversion"
      }
    ]
  },
  "twitter": {
    "card": "summary_large_image",
    "title": "Event Properties Conversion",
    "description": "Learn how Datablit converts event properties to snake_case and flattens nested objects. Understand property naming conventions and data transformation rules.",
    "images": [
      "/icon.svg"
    ],
    "creator": "@Datablit_cdp"
  },
  "robots": {
    "index": true,
    "follow": true
  }
};


# Getting Started with Event Properties Conversion

1. All event properties will be converted into snake_case string
2. Nested object will be flatten and key will combined with parent key except fields inside context, properties and traits

**Sample event**

```json
{
  "anonymousId": "b67cd1a3-9ad1-4df8-8ceb-cfc7e542b56e",
  "messageId": "b5f60cba-7f30-4d41-9901-589601e1e539",
  "type": "track",
  "userId": "1",
  "context": {
    "library": {
      "name": "analytics-kotlin",
      "version": "1.19.1"
    },
    "instanceId": "54e25ce3-e14c-4c51-8bd6-eaea9ab4bc66",
    "app": {
      "name": "Test app",
      "build": "602"
    },
    "device": {
      "id": "b67cd1a3-9ad1-4df8-8ceb-cfc7e542b56e"
    }
  },
  "receivedAt": null,
  "timestamp": "2025-02-24T05:13:03.708Z",
  "originalTimestamp": "2025-02-24T05:13:03.708Z",
  "event": "Test",
  "properties": {
    "User Bucket": "0"
  },
  "traits": null
}
```

**After conversion**

```json
{
  "anonymous_id": "b67cd1a3-9ad1-4df8-8ceb-cfc7e542b56e",
  "message_id": "b5f60cba-7f30-4d41-9901-589601e1e539",
  "type": "track",
  "user_id": "1",
  "library_name": "analytics-kotlin",
  "library_version": "1.19.1",
  "instance_id": "54e25ce3-e14c-4c51-8bd6-eaea9ab4bc66",
  "app_name":"Test app",
  "app_build": "602",
  "device_id" : "b67cd1a3-9ad1-4df8-8ceb-cfc7e542b56e",
  "received_at": null,
  "timestamp": "2025-02-24T05:13:03.708Z",
  "original_timestamp": "2025-02-24T05:13:03.708Z",
  "event": "Test",
  "user_bucket": "0",
  "traits": null
}
```
