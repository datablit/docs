...
To create a new Experiment, open the Experiment Tab in Events section.
...
<Table
  ...
  data={[
    [
      ...
      "Yes",
      <>
        By default, its value is set to active. To disable it, change the value
        to disabled. Once the experiment is disabled, it will return the{" "}
        <b>control</b> variant for all entities.
      </>,
    ],
    [
      "Percentage",
    ...
    [
      ...
      "No",
      <>
        If you want to assign a specific test entity ID to a particular variant,
        enter the ID, select the desired variant, and click "Assign." This will
        force the given ID to always receive the selected variant, regardless of
        the experiment's configuration or percentage rollout.
      </>,
    ],
  ]}
/>
