...
### Sample curl
```curl
curl --location 'https://event.datablit.com/v1/batch' \
    ...
    "batch": [
        {
            ...
            "userId": "user1",
            "context": {},
            "timestamp": "2025-10-13T02:24:03.708Z",
...
<Table
  ...
  data={[
    ...
    ["messageId", "string", "no", "Automatically collected by Datablit, a unique identifier for each message that lets you find an individual message across the API."],
...
``` curl
curl --location 'https://console.datablit.com/api/1.0/rule/eval' \
--header 'apiKey: <your_api_key>' \
--header 'Content-Type: application/json' \
--data '{
    "key" : "disable_cod",
...
```curl
curl --location 'https://console.datablit.com/api/1.0/experiment/variant?expId=01K7T83YWG4XA20565VT1PXD04&entityId=user4' \
--header 'apiKey: <your_api_key>'
```
### Query params detail
